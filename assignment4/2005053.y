...
func_definition : type_specifier ID LPAREN parameter_list RPAREN {
		...
		compound_statement
		{
			...
			errorflag=symtable.Insert($2->nameGetter(),"ID");
			SymbolInfo *tmp=symtable.LookUp($2->nameGetter());
			...
			if(errorflag)
			{   
				tmp->retTypeSetter($1->varTypeGetter());
...
declaration_list : declaration_list COMMA ID
 		  ...
 		  | declaration_list COMMA ID LTHIRD CONST_INT RTHIRD
		  {
			...
			//{
			    ...
			    temp->varTypeSetter(varType);
				//cout<<"Bye";
				temp->sizeSetter(atoi($5->nameGetter().c_str()));
 		  ...
 		  | ID LTHIRD CONST_INT RTHIRD
		  {
			...
			//{
			    ...
			    temp->varTypeSetter(varType);
				//cout<<"Hello";
				temp->sizeSetter(atoi($3->nameGetter().c_str()));
			//}
